...
- Added rule `"unicorn/no-nested-ternary": "off",` to avoid clashes with Prettier.
...
- Migrated the configuration from legacy .eslintrc format to ESLint 9 flat config.
- Consumers must now import the configuration from eslint.config.js (or another flat config file) instead of using extends in .eslintrc.

### Changed

- Updated the configuration to use the flat-config versions of @typescript-eslint, eslint-plugin-import, and eslint-plugin-unicorn.
- Replaced eslint-plugin-css-import-order with a custom local/css-import-order rule.
- Replaced eslint-plugin-typescript-sort-keys with eslint-plugin-sort-properties.
= Updated the TypeScript empty-interface rule to @typescript-eslint/no-empty-object-type.
= Added the custom CSS import-order rule to the published package.
= Removed unused dependencies: eslint-config-prettier, eslint-plugin-babel, and eslint-plugin-unused-imports.
- Updated the package metadata and documentation for ESLint flat config.
